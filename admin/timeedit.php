...
if ((!isset($_SESSION['valid_admin'])) && (!isset($_SESSION['time_admin_valid_user']))) {
...
elseif ($request == 'POST') {
	...
	$post_datetime = DateTime::createFromFormat($datefmt.'|', $post_date);
	$post_date_errors = DateTime::getLastErrors();
	...
	if (empty($post_date) || $post_date_errors['error_count'] > 0) {
		$evil_post = '1';
	...
	if (isset($evil_post)) {
		...
		if (isset($_POST['tmp_var'])) {
			...
			for ($x=0;$x<$final_num_rows;$x++) {
				...
				$edit_datetime[$x] = DateTime::createFromFormat($datefmt." ".$timefmt, $post_datetime->format($datefmt)." ".$edit_time_textbox[$x]);
				$edit_datetime_errors[$x] = DateTime::getLastErrors();
           ...
           (`inout` = '".$final_inout[$x]."')";
				...
				if (!empty($edit_time_textbox[$x])) {
					...
					*/
					...
					$tmp_timestamp = $post_datetime->getTimestamp();
					$tmp_calc = $post_datetime->getTimestamp() + 86400;
					...
					if ($edit_datetime_errors[$x]['error_count'] > 0) {
						$evil_time = '1';
					}
				}
			}
			...
			if (isset($evil_time)) {
				...
				echo "                <th class=rightside_heading nowrap halign=left colspan=4><img src='../images/icons/clock_edit.png' />&nbsp;&nbsp;&nbsp;Edit
                  Time for $post_username on ".$post_datetime->format($datefmt)."</th></tr>\n";
				echo "              <tr><td height=15></td></tr>\n";
			...
			elseif (!isset($evil_time)) {
				...
				echo "                <th class=rightside_heading nowrap halign=left colspan=5><img src='../images/icons/clock_edit.png' />&nbsp;&nbsp;&nbsp;Edited
                  Time for $post_displayname on ".$post_datetime->format($datefmt)."</th></tr>\n";
				echo "              <tr><td height=15></td></tr>\n";
				...
				for ($x=0;$x<$final_num_rows;$x++) {
					if ($edit_time_textbox[$x] != '') {
						...
						// configure timestamp to insert/update //
						$new_tstamp[$x] = $edit_datetime[$x]->getTimestamp();
           ...
           ('".$user."', '".$time_tz_stamp."', '".$final_mysql_timestamp[$x]."', '".$new_tstamp[$x]."', '".$post_why."', '".$final_username[$x]."')";
			...
			$timestamp = $post_datetime->getTimestamp();
		...
		if (!isset($time_set)) {
			...
			echo "              <tr>\n";
			echo "                <td class=table_rows width=20 align=center><img src='../images/icons/cancel.png' /></td><td class=table_rows_red>
                    No time for was found in the system for $post_username on ".$post_datetime->format($datefmt).".</td></tr>\n";
			echo "            </table>\n";
			echo "            <br />\n";
			echo "            <table align=center class=table_border width=60% border=0 cellpadding=3 cellspacing=0>\n";
			echo "              <tr>\n";
		...
		if (isset($time_set)) {
			...
			for ($x=0;$x<$num_rows;$x++) {
				...
				$row_color = ($row_count % 2) ? $color1 : $color2;
				$time[$x] = date("$timefmt", $mysql_timestamp[$x]);
