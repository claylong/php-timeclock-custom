...
/*
	if (!empty($from_date)) {
		...
		$from_timestamp = strtotime($from_date . " " . $report_start_time);
		$from_date = $_POST['from_date'];
	}

	if (!empty($to_date)) {
		$to_date = "$to_month/$to_day/$to_year";
		$to_timestamp = strtotime($to_date . " " . $report_end_time) + 60;
		$to_date = $_POST['to_date'];
	}
*/
				...
				if (isset($tmp_info_date)) {
					if ($tmp_info_date == $info_date[$y]) {
						if (empty($punchlist_in_or_out[$y])) {
							...
							$punch_cnt++;
							if ($y == $info_cnt - 1) {
								if (($info_timestamp[$y] <= $rpt_stamp) && ($rpt_stamp < ($to_timestamp)) && ($x_info_date[$y] == $rpt_date)) {
									if ($status == "in") {
