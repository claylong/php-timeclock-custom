...
if ($request == 'GET') {
	...
	if ($username_dropdown_only == "yes") {
		...
		while ($row=mysql_fetch_array($result)) {
			...
			$tmp_displayname = stripslashes("".$row['displayname']."");
			echo "                    <option value='$tmp_empfullname'>$tmp_displayname</option>\n";
		}
	...
	$selected_username = stripslashes($_POST['user_name']);
	$from_date = $_POST['from_date'];
	$to_date = $_POST['to_date'];
	@$tmp_display_ip = $_POST['tmp_display_ip'];
	@$tmp_csv = $_POST['csv'];
	
	$selected_username = addslashes($selected_username);

	$from_datetime = DateTime::createFromFormat($datefmt.'|', $from_date);
	$from_date_errors = DateTime::getLastErrors();
	
	$to_datetime = DateTime::createFromFormat($datefmt.'|', $to_date);
	$to_date_errors = DateTime::getLastErrors();
	
	$selected_displayname = $selected_username;
	
	// begin post error checking //

	if ($selected_username != "All") {
		$query = "select * from ".$db_prefix."employees where empfullname = '".$selected_username."'";
		$result = mysql_query($query);
	...
	$selected_displayname = stripslashes($displayname);
	$selected_username = stripslashes($selected_username);
	...
	if (!isset($evil_post)) {
		if (empty($from_date) || $from_date_errors['error_count'] > 0) {
			$evil_post = '1';
			if ($use_reports_password == "yes") {
				include '../admin/topmain.php';
			} else {
	...
	if (!isset($evil_post)) {
		if (empty($to_date) || $to_date_errors['error_count'] > 0) {
			$evil_post = '1';
			if ($use_reports_password == "yes") {
				include '../admin/topmain.php';
			} else {
	...
	if (isset($evil_post)) {
		...
		if ($username_dropdown_only == "yes") {
			...
			while ($row=mysql_fetch_array($result)) {
				...
				$displayname_tmp =  stripslashes("".$row['displayname']."");
				echo "                    <option value='$empfullname_tmp'>$displayname_tmp</option>\n";
			}
			...
			echo "              <tr><td class=table_rows height=25 width=20% style='padding-left:32px;' nowrap>Choose Username:</td><td colspan=2 width=80%
                      ...
                      <select name='user_name' onfocus='user_names();'>
                          <option selected value='$selected_username'>$selected_displayname</option>\n";
			echo "                      </select></td></tr>\n";
		}
		echo "              <tr><td class=table_rows style='padding-left:32px;' width=20% nowrap>From Date: ($tmp_datefmt)</td><td
                      style='color:red;font-family:Tahoma;font-size:10px;padding-left:20px;' width=80% >
...
/*
	if (!empty($from_date)) {
		...
		$from_datetime = strtotime($from_date) - @$tzo;
		$from_date = $_POST['from_date'];
	}

	if (!empty($to_date)) {
		$to_date = "$to_month/$to_day/$to_year";
		$to_datetime = strtotime($to_date) + 86400 - @$tzo;
		$to_date = $_POST['to_date'];
	}
*/

	$to_timestamp = $to_datetime->getTimestamp() + 86400;
	...
	$tmp_fullname = stripslashes($selected_username);
 ...
 Users: All";
	...
	if (!empty($tmp_csv)) {
		echo "               <tr class=notprint><td width=80%></td><td nowrap style='font-size:9px;color:#000000;'><a style='color:#27408b;font-size:9px;
                         ...
                         href=\"get_csv.php?rpt=timerpt&display_ip=$tmp_display_ip&csv=$tmp_csv&office=$office_name&group=$group_name&fullname=$selected_username&from=$from_timestamp&to=$to_timestamp&tzo=$tzo\">Download CSV File</a></td></tr>\n"; 
	}
	echo "            </table>\n";
	...
	if (strtolower($user_or_display) == "display") {
		...
		if (($office_name == "All") && ($group_name == "All") && ($selected_username == "All")) {

			$query = "select empfullname, displayname from ".$db_prefix."employees WHERE tstamp IS NOT NULL order by displayname asc";
			$result = mysql_query($query);

		} elseif ((empty($office_name)) && (empty($group_name)) && ($selected_username == 'All')) {
			...
			$query = "select empfullname, displayname from ".$db_prefix."employees WHERE tstamp IS NOT NULL and empfullname = '".$selected_username."' order by
                  displayname asc";
			$result = mysql_query($query);

		} elseif (($office_name != "All") && ($group_name == "All") && ($selected_username == "All")) {
			...
			$query = "select empfullname, displayname from ".$db_prefix."employees where office = '".$office_name."' and groups = '".$group_name."' and
                  empfullname = '".$selected_username."' and tstamp IS NOT NULL order by displayname asc";
			$result = mysql_query($query);
		}
		...
		if (($office_name == "All") && ($group_name == "All") && ($selected_username == "All")) {
			...
			$query = "select empfullname, displayname from ".$db_prefix."employees where office = '".$office_name."' and groups = '".$group_name."' and
                  empfullname = '".$selected_username."' and tstamp IS NOT NULL order by empfullname asc";
			$result = mysql_query($query);
		}
	}
	...
	for ($x=0;$x<$employees_cnt;$x++) {
		...
		if (($employees_empfullname[$x] == $selected_username) || ($selected_username == "All")) {
